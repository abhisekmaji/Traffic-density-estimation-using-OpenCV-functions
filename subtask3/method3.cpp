...
/*-------global variables and functions for threading---------*/

vector<unsigned long> mag_thread(num_splits,0);
vector<unsigned long> mag_queue(num_splits,0);
vector<unsigned long> mag_dynamic(num_splits,0);



void cal_magnitude_new(Mat diff, int index){
    ul norm = 0;
    ...
    mag_thread[index] = norm;
    mag_queue[index] = mag;
}

struct arg_struct {
    Mat arg1;   //cropped image
    Mat arg2;   //empty image
    int arg3;   //index of the thread
};

void *process(void *arguments){
    struct arg_struct *args = (struct arg_struct *)arguments;
    Mat cropped = args-> arg1;
    Mat empty = args-> arg2;
    int index = args-> arg3;
    Mat dst;
    absdiff(cropped,empty,dst);
    cal_magnitude_new(dst,index);
    
    pthread_exit(NULL);
}

float threading(int num_threads, Mat cropped , Mat empty ,vector<Rect> &blocks){
    phtread_t threads[num_threads];
    struct arg_struct args[num_threads];
    int check;
    for(int i=0; i<num_threads, i++){
        args[i].arg1 = cropped(blocks[i]);
        args[i].arg2 = empty(blocks[i]);
        args[i].arg3 = i;
        check = pthread_create(&threads[i],NULL,&process,(void *)&args[i]);
        if (check){
    ...
    return (mag1*1.0)/(mag2*1.0);    
}

void split_img(int s, vector<Rect> &blocks){
    int width = img.cols;
    int height = img.rows;
    int block_width = ( width / s )+ 1;
    int i;
    Mat cropped;
    for(i = 0; i<= block_width*(s-1); i+= block_width){
        Rect grid(i,height,block_width,height);
        blocks.push_back(cropped);
    }
    Rect grid(i,height,width-block_width,height);
    blocks.push_back(cropped);
}
...
int main(int argc, char** argv){
    ...
    while(k<1000000){
        ...
        if(cnt == 15){
            /*implementing threading to calculate the queue density*/            
            a[k] = threading(num_splits, cropped, empty, blocks);            
            //done implementation
